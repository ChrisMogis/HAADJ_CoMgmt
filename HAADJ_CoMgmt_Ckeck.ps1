...
.DESCRIPTION
    ...
    V1.0.3
    Ajout d'un tag dans le registre pour le controle d'execution lorsque le poste est hors connexion
...
#Detection registre pour utilisateur connecté hors site ou deconnecté
$RegKeyHaadj = "HKCU:\\Software\\HAADJCheck"
$RegKeyHaadjName = "HAADJResult"
$ScriptHAADJResult = (Get-Item $RegKeyHaadj -EA Ignore).Property -contains $RegKeyHaadjName
if ($ScriptHAADJResult -ne "True")
{
...
If ($HAADJ -eq 'Yes' -and $CoMgnt -eq 'Yes' -and $WinDef -eq 'Yes')
    {
    try {
        #Creation de l'objet HAADJCheck dans le registre
        New-Item –Path "HKCU:Software" –Name "HAADJCheck"
        #Write-Host "Envoie des résultats au service IT" -ForegroundColor Yellow
        Copy-Item $logfilepath -Destination $LogShare -Force
	    #Creation d'un tag dans le registre
	    New-ItemProperty -Path "HKCU:\\Software\\HAADJCheck" -Name "HAADJResult" -Value "OK"  -PropertyType "ExpandString"
    	}
    catch
        {
        Write-host "Erreur de copie, réexecution du script"
        }
    }
...
#Detection du fichier contenant les resultats du check 
    ...
    If ($TestPathCSV -eq "True")
        ...
        {
        Write-Host "Error copie de fichier"
        }
    }
}
